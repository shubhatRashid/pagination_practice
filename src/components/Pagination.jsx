    ...
    return (
        <div style={{display:'flex',width:'100%',gap:"25px",justifyContent:'space-around',alignItems:'center'}}>
            ...
            <div style={{display:'flex',gap:"25px"}}>
                ...
                {currPage+1 < lastPage &&<button style={styles.numberStyle} onClick={() => jumpTopage(currPage+1)}>{currPage+1}</button>}
                {currPage+2 < lastPage &&<button style={styles.numberStyle} onClick={() => jumpTopage(currPage+2)}>{currPage+2}</button>}
                <span>...</span>
            </div>
